...
import {
  ...
  type Item,
  type TaxCode,
  type CreateProformaInvoiceLine,
} from "@/lib/api";
...
export function NewProformaClient({
  customers,
  items,
  taxCodes,
}: {
  ...
  const [validUntil, setValidUntil] = useState(() =>
    plusDaysISO(new Date().toISOString().slice(0, 10), 30),
  );
  ...
  async function submit() {
    ...
    const apiLines: CreateProformaInvoiceLine[] = [];
    ...
    setBusy(true);
    try {
      const res = await api.createProformaInvoice({
        customerId,
        issueDate,
        validUntil,
      ...
      router.push(`/app/proforma-invoices/${res.proformaInvoice.id}`);
    } catch (err) {
  ...
  return (
    <main className="container-p py-10">
      ...
      <PageHeader
        ...
        description="Draft a proforma for your customer. Save as draft, then send — a proforma number (PF-YYYY-NNNN) gets allocated at that point. When the deal firms up, convert it to a real invoice."
      />
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div>
          <label htmlFor="ref" className="block text-caption uppercase tracking-wide text-text-tertiary">
            Reference (optional)
          </label>
          <input
            id="ref"
            ...
            placeholder="RFQ-42, LC number, opportunity name"
            className="input mt-1.5"
          />
        </div>
        <div>
          <label htmlFor="issue" className="block text-caption uppercase tracking-wide text-text-tertiary">
            Issue date
          </label>
          <input
            id="issue"
        ...
        <div>
          <label htmlFor="valid" className="block text-caption uppercase tracking-wide text-text-tertiary">
            Valid until
          </label>
          <input
            id="valid"
                ...
                <tr key={l.id}>
                  ...
                  <td className="px-3 py-2">
                    <input
                      ...
                      placeholder="What's being supplied"
                      className="input text-small"
                    />
                  </td>
                  <td className="px-3 py-2">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        <div>
          ...
          <textarea
            ...
            placeholder="Internal context — won't appear on the proforma"
            className="input mt-1.5"
          />
        </div>
        <div>
          <label htmlFor="terms" className="block text-caption uppercase tracking-wide text-text-tertiary">
            Terms
          </label>
          <textarea
            id="terms"
            ...
            placeholder="Payment terms, delivery window, customs details — printed on the proforma"
            className="input mt-1.5"
          />
        </div>
      </section>
      ...
      <section className="mt-6 flex items-center justify-between gap-4">
        <p className="text-small text-text-secondary">
          Saved as a draft. Send to allocate a proforma number, then convert to an invoice when the deal firms up.
        </p>
