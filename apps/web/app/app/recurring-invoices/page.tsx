...
import type { RecurringInvoiceListRow } from "@/lib/api";
import { PageHeader } from "@/components/app/page-header";
import { formatDate } from "@/lib/format";
import { RecurringInvoiceActions } from "./recurring-actions-client";

export const metadata: Metadata = { title: "Recurring invoices" };

async function fetchList(): Promise<RecurringInvoiceListRow[] | null> {
  const res = await fetch(
    `${process.env.INTERNAL_API_URL ?? "http://api:4000"}/recurring-invoices`,
    { headers: { cookie: cookies().toString() }, cache: "no-store" },
  );
  if (!res.ok) return null;
  return ((await res.json()) as { recurringInvoices: RecurringInvoiceListRow[] }).recurringInvoices;
}

function statusPill(row: RecurringInvoiceListRow) {
    ...
    <main className="container-p py-10">
      <div className="flex items-start justify-between gap-4">
        <PageHeader
          ...
          description="Monthly retainers, rent, and subscriptions. Each template auto-generates a draft invoice on its scheduled day — review and post to send."
        />
        <Link href="/app/recurring-invoices/new" className="btn-primary inline-flex items-center gap-2 text-small">
      ...
      {!rows || rows.length === 0 ? (
        <div className="mt-6 rounded-card border-hairline border-border bg-surface-elevated p-10 text-center">
          ...
          <p className="mt-1 text-caption text-text-tertiary">
            Set up a monthly schedule for customers you invoice on repeat — the worker drops a draft on each run date.
          </p>
          <Link href="/app/recurring-invoices/new" className="btn-primary mt-4 inline-flex items-center gap-2 text-small">
            <Plus className="h-3.5 w-3.5" aria-hidden />
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td className="px-4 py-3 text-text-secondary">{r.customerName}</td>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums text-text-secondary">
                      {r.generatedCount}
                      {r.lastGeneratedInvoiceId && (
                        <Link
                          href={`/app/invoices/${r.lastGeneratedInvoiceId}`}
                    ...
                    <td className="px-4 py-3 text-right">
                      <RecurringInvoiceActions id={r.id} isActive={r.isActive} />
                    </td>
                  </tr>
                );
              })}
