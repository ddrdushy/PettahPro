...
export function RecurringBillActions({ id, isActive }: { id: string; isActive: boolean }) {
  ...
  return (
    <div className="inline-flex items-center gap-1">
      <button
        ...
        onClick={() =>
          run("gen", async () => {
            const res = await api.generateRecurringBillNow(id);
        ...
        title="Generate a draft bill from this template now"
      >
      ...
      <button
        ...
        onClick={() =>
          run(
            "toggle",
            isActive ? () => api.pauseRecurringBill(id) : () => api.resumeRecurringBill(id),
          )
        }
        className="btn-ghost inline-flex items-center gap-1 px-2 py-1 text-caption disabled:opacity-50"
      >
