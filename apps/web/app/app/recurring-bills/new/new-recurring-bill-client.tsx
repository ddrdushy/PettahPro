...
export function NewRecurringBillClient({
  suppliers,
  items,
  taxCodes,
  expenseAccounts,
  ...
  const [supplierBillNumberTemplate, setSupplierBillNumberTemplate] = useState("");
  ...
  function pickItem(lineId: string, itemId: string) {
    ...
    updateLine(lineId, {
      ...
      unitPrice: item ? (Number(item.buyPriceCents ?? 0) / 100).toString() : "0",
    });
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await api.createRecurringBill({
        ...
        supplierBillNumberTemplate: supplierBillNumberTemplate.trim() || undefined,
  ...
  return (
    <main className="container-p py-10">
      <div className="mb-4">
        <Link href="/app/recurring-bills" className="btn-link text-small">
      ...
      <PageHeader
        ...
        description="Create a template that generates a draft bill every month on the day you pick. You'll still review and post each draft — nothing posts automatically."
      />
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. Monthly rent — Colombo office"
              ...
              <div className="sm:col-span-2">
                <Label htmlFor="billNumberTemplate">Supplier bill number template (optional)</Label>
                <input
                  id="billNumberTemplate"
                  value={supplierBillNumberTemplate}
                  onChange={(e) => setSupplierBillNumberTemplate(e.target.value)}
                  placeholder="e.g. RENT-{YYYY}{MM}"
                  className="input mt-1.5 w-full"
                />
                <p className="mt-1 text-caption text-text-tertiary">
                  Tokens: <code>{"{YYYY}"}</code> <code>{"{YY}"}</code> <code>{"{MM}"}</code> <code>{"{SEQ}"}</code>.
                  Leave blank if the supplier sends you a different number every month — paste it when the invoice arrives.
                </p>
              </div>
              <div className="sm:col-span-2">
          ...
          <section className="rounded-card border-hairline border-border bg-surface-elevated">
            ...
            <div className="divide-y-hairline divide-border">
              {lines.map((l) => (
                <div
                  key={l.id}
                  className="grid gap-3 px-5 py-4 md:grid-cols-[1fr_1fr_80px_100px_70px_100px_1fr_40px]"
                >
                  ...
                  <select
                    ...
                    onChange={(e) => updateLine(l.id, { expenseAccountId: e.target.value })}
                    className="input"
                    title="Expense account — required for free-form (non-item) lines"
                  >
    ...
    <div className="flex items-center justify-between">
      ...
      <dd
        className={`tabular-nums ${
          strong ? "font-medium text-charcoal" : dim ? "text-text-tertiary" : "text-charcoal"
        }`}
      >
        {value}
      </dd>
