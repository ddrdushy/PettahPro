...
export function NewPurchaseOrderClient({
  ...
  async function submit() {
    ...
    const apiLines: CreatePurchaseOrderLine[] = [];
    ...
    try {
      ...
      router.push(`/app/purchase-orders/${res.purchaseOrder.id}`);
    } catch (err) {
  ...
  return (
    <main className="container-p py-10">
      ...
      <PageHeader
        ...
        description="Draft a commitment to your supplier. Save as draft first, then send when ready — a PO number gets allocated at that point."
      />

      <section className="mt-6 grid gap-4 md:grid-cols-2">
        <div>
          <label htmlFor="supplier" className="block text-caption uppercase tracking-wide text-text-tertiary">Supplier</label>
          <select id="supplier" value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="input mt-1.5">
        ...
        <div>
          ...
          <input id="ref" type="text" value={reference} onChange={(e) => setReference(e.target.value)} placeholder="Internal project code or tag" className="input mt-1.5" />
        </div>
        <div>
        ...
        <div>
          <label htmlFor="expected" className="block text-caption uppercase tracking-wide text-text-tertiary">Expected delivery (optional)</label>
          <input id="expected" type="date" value={expectedDeliveryDate} onChange={(e) => setExpectedDeliveryDate(e.target.value)} className="input mt-1.5" />
        </div>
      </section>
            ...
            {lines.map((l, idx) => {
              ...
              return (
                <tr key={l.id}>
                  ...
                  <td className="px-3 py-2">
                    <input type="text" value={l.description} onChange={(e) => updateLine(l.id, { description: e.target.value })} placeholder="What you're ordering" className="input text-small" />
                  </td>
                  <td className="px-3 py-2">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <textarea id="terms" rows={3} value={terms} onChange={(e) => setTerms(e.target.value)} placeholder="Delivery, payment, warranty — printed on the PO" className="input mt-1.5" />
        </div>
      </section>

      <section className="mt-6 flex items-center justify-between gap-4">
        <p className="text-small text-text-secondary">Saved as a draft. Send, acknowledge, cancel, or convert from the next screen.</p>
