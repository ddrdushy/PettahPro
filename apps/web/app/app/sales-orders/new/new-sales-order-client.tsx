...
export function NewSalesOrderClient({
  customers,
  items,
  taxCodes,
}: {
  ...
  async function submit() {
    ...
    const apiLines: CreateSalesOrderLine[] = [];
    ...
    try {
      ...
      router.push(`/app/sales-orders/${res.salesOrder.id}`);
    } catch (err) {
  ...
  return (
    <main className="container-p py-10">
      ...
      <PageHeader
        ...
        description="Record a customer's commitment to buy. Confirm when ready — an SO number gets allocated at that point."
      />
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <input id="ref" type="text" value={reference} onChange={(e) => setReference(e.target.value)} placeholder="Project code, opportunity name" className="input mt-1.5" />
        </div>
        <div>
          <label htmlFor="order" className="block text-caption uppercase tracking-wide text-text-tertiary">Order date</label>
          <input id="order" type="date" value={orderDate} onChange={(e) => setOrderDate(e.target.value)} className="input mt-1.5" />
        </div>
        <div>
          <label htmlFor="ship" className="block text-caption uppercase tracking-wide text-text-tertiary">Expected ship (optional)</label>
          <input id="ship" type="date" value={expectedShipDate} onChange={(e) => setExpectedShipDate(e.target.value)} className="input mt-1.5" />
        </div>
        <div className="md:col-span-2">
          <label htmlFor="cpo" className="block text-caption uppercase tracking-wide text-text-tertiary">Customer's PO number (optional)</label>
          <input id="cpo" type="text" value={customerPoNumber} onChange={(e) => setCustomerPoNumber(e.target.value)} placeholder="Their PO referencing your quote/order" className="input mt-1.5" />
        </div>
      </section>
            ...
            {lines.map((l, idx) => {
              ...
              return (
                <tr key={l.id}>
                  ...
                  <td className="px-3 py-2">
                    <input type="text" value={l.description} onChange={(e) => updateLine(l.id, { description: e.target.value })} placeholder="What's being ordered" className="input text-small" />
                  </td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2">
                    <select value={l.taxCodeId} onChange={(e) => updateLine(l.id, { taxCodeId: e.target.value })} className="input text-small">
                      ...
                      {saleTaxCodes.map((t) => (<option key={t.id} value={t.id}>{t.code} ({(t.rateBps / 100).toFixed(2)}%)</option>))}
                    </select>
                  </td>
          ...
          <tfoot className="bg-surface-recessed/50">
            ...
            {computed.tax > 0 && (
              <tr>
                ...
                <td className="px-3 py-2 text-right tabular-nums text-text-secondary" colSpan={2}>Tax {formatLKR(computed.tax)}</td>
              </tr>
            )}
            <tr>
              <td colSpan={7} className="px-3 py-3" />
              <td className="px-3 py-3 text-right tabular-nums font-medium text-charcoal" colSpan={2}>Total {formatLKR(computed.total)}</td>
            </tr>
          </tfoot>
        </table>
      </section>
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <textarea id="terms" rows={3} value={terms} onChange={(e) => setTerms(e.target.value)} placeholder="Delivery, payment, warranty — printed on the SO" className="input mt-1.5" />
        </div>
      </section>

      <section className="mt-6 flex items-center justify-between gap-4">
        <p className="text-small text-text-secondary">Saved as a draft. Confirm, cancel, or convert from the next screen.</p>
