...
| Approval engine (generic) | #74 + #75 (expense_claim wired) | — | 0 | Roadmap #43 + #43a. Consumes `approval_policies` (from #26), snapshots steps on submit into `approval_request_steps` so in-flight requests bind to the rules in effect at submit time. `/approvals` queue supports `mine` / `submitted_by_me` / `all` scopes + deep-links to the source domain (JE drafts, expense claims). `approval.decide` permission (Owner/Admin/Accountant). SOD is belt-and-braces: `recordDecision` throws `SELF_APPROVAL` regardless of approver list. Wired domains: `journal_entry` (#74), `expense_claim` (#75). 43b–43e add bill / PO / payroll+bonus / final settlement. See fragile areas §3. |
