...
| Approval engine (generic) | #74 | — | 0 | Roadmap #43. Consumes `approval_policies` (from #26), snapshots steps on submit into `approval_request_steps` so in-flight requests bind to the rules in effect at submit time. `/approvals` queue supports `mine` / `submitted_by_me` / `all` scopes. `approval.decide` permission (Owner/Admin/Accountant). SOD is belt-and-braces: `recordDecision` throws `SELF_APPROVAL` regardless of approver list. JE is the only wired domain today; 43a–43e add expense claim / bill / PO / payroll+bonus / final settlement. See fragile areas §3. |
