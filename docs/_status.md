...
Last updated: 2026-04-23 (PR #72 — #31 customer portal hardening). Closes three v1 gaps: brute-force protection on `/portal/auth/verify` (10/hr per email), portal-activity audit events (`portal.login`, `portal.logout`, `portal.access_toggled`), and per-customer `portal_enabled` toggle (default true) that revokes access without archiving the customer. No new typecheck errors; baselines held (api: 29, web: 10, db: 1). Previously: PR #71 — #31 customer portal. No new known bugs; no shipped regressions. Typecheck baseline held (api: 29 observed, web: 10, db: 1). PR #71 adds a new `portal_otps` table with 5 SECURITY DEFINER helpers (`portal_find_customers_by_email`, `portal_mint_otp`, `portal_find_otp`, `portal_consume_otp`, `portal_resolve_session`), a portal session realm backed by Redis (`portal-session:` prefix, separate from admin `pp_session`), new `/portal/*` API routes (auth + invoices + statement + payments + recurring), and the `/portal/*` Next.js App Router surface (login → verify → invoices / statement / payments / recurring + invoice detail with PDF download). Modules touched in §5 below: **Customer portal (new)**.
