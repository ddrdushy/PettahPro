...
| Customer portal | #71 | — | 0 | Email+OTP self-serve. Separate realm from admin app (different cookie + Redis prefix + path-scoped `/portal` + distinct onRequest hook). OTP is 6-digit, SHA-256 hashed at rest, 10-min expiry, 5-per-email-per-hour rate limit. Cross-tenant enumeration-safe: always returns 200 at request-otp, mints one OTP per matching (tenant, customer) with the same plaintext code, and verify handles ambiguity via 409 `AMBIGUOUS_TENANT`. All reads double-scoped (`withTenant()` + explicit `customer_id =` predicate). Drafts return 404. PDF download reuses `InvoicePDF` renderer via portal-scoped route. v1 is read-only — pay-online deferred. |
