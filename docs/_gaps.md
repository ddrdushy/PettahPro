...
Updated 2026-04-26 after the **configurable plan catalogue** PR — closes the "Read-only for this PR; plan editing (add/edit/archive) is a later ticket" todo from #61. Plans now have `is_archived` (migration `90-plans-archive.sql`); super-admin-only `POST /platform/plans` / `PATCH /platform/plans/:id` / `POST /platform/plans/:id/archive` / `unarchive` endpoints with audit (`platform.plan.{created,updated,archived,unarchived}`); plan editor drawer at `/platform/plans` with chip-input feature toggles, "Unlimited" cap toggles, and grandfathering-safe archive flow. Earlier: PR #115 docs-sync sweep that documented PRs #61–73 (L2 pricing-plan engine — foundation, gating, trial/grace, self-serve change, quotas, plan-aware sidebar, page-level + inline upgrade CTAs, app-wide trial/grace banner, per-tenant quota overrides, custom-contract surface on settings, plan-aware POS error banner) and #57 (L1 v1 third slice — operator impersonation: consent-gated, time-boxed, dual-actor audit). Earlier: PR #56 L1 v1 second slice (multi-role platform staff), PR #55 L1 v1 first slice (platform-user MFA), PR #54 L1 v0 (super-admin console), PR #53 D1 (email delivery for immediate notifications).
