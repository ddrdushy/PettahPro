...
Last updated: 2026-04-23 after PR #70 "#45 notification digest windows" (follows PR #69 hardening pass + PR #67 POS terminal + PR #68 commission engine). **All must-haves, all original should-haves, #28 POS, #29 commission engine, and now #45 digest windows all shipped.** Remaining tracked backlog: one deferred follow-up from PR #63 (#43 approval engine wiring) plus the Nice-to-have list.
...
Current recommendation: compliance + convenience + role enforcement + FX revaluation + POS + commission engine + digest windows all done, and PR #69 tightened the edges on everything shipped. Nothing user-visible is a hard gap. Next work falls in two camps:
...
2. **Pick from Nice-to-have** — **#36 inventory category hierarchy** is the only remaining S-sized cheap win (organization for tenants with >50 SKUs). Beyond that the list is all M/L — #30 purchase requisition, #31 customer portal, #32 document attachments, #33 document template builder, #34 batch/serial/expiry tracking, #35 kit/bundle items, #38 petty cash float, #39 attendance capture, #40 dual depreciation.

Leaning toward **#36 inventory category hierarchy next** as the S-sized cheap win to clear before tackling #43. Alternately, **#31 customer portal** (L) is the single highest-leverage Nice-to-have — lets tenants drop the "email a statement" step and lets customers self-serve. If we're picking a big one, that's the pick.
